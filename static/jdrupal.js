...
function drupal_init() {
  try {
    ...
    //console.log('JDRUPAL init resetting Drupal.user');
...
function entity_load(entity_type, ids, options) {
    ...
    //console.log('entity caching enabled?', caching_enabled);
    ...
    if (_services_queue_already_queued(entity_type, 'retrieve', entity_id, 'success')) {
      if (caching_enabled) {
        ...
        if (entity) {
          //console.log('JDRUPAL read entity from cache');
      ...
      success: function(data) {
        //try {
          ...
          if (entity_caching_enabled(entity_type, entity_get_bundle(entity_type, entity))) {
            ...
            //console.log('JDRUPAL saved entity to cache');
          }
  ...
  try {
    ...
    if (entity) {
      ...
      if (entity_has_expired(entity_type, entity)) {
        ...
        var drupalgap = false; // XXX: added by me
...
function _entity_get_expiration_time(entity_type, entity) {
  //try {
    ...
    if (entity_caching_enabled(entity_type, bundle)) {
      ...
      if (typeof cache.entity.expiration !== 'undefined') {
        expiration = cache.entity.expiration;
      }
      if (typeof cache.entity.entity_types !== 'undefined') {
        if (
...
Drupal.services.call = function(options) {
  try {
    ...
    var url = Drupal.settings.site_path +
              Drupal.settings.base_path + ''; // removed ?q=
    ...
    services_get_csrf_token({
        ...
        success: function(token) {
          try {
            ...
            //console.log('JDRUPAL setting X-CSRF-Token header to', token);
            if (token) {
              request.setRequestHeader('X-CSRF-Token', token);
            }

            // XXX: makes requests work when logged in w/ cookies b/c the request uses the cookies for that domain
...
function services_get_csrf_token(options) {
  try {
    ...
    options.token_url = Drupal.settings.site_path + Drupal.settings.base_path + 'services/session/token?_z=' + Math.random(); // removed ?q=

    module_invoke_all('csrf_token_preprocess', options);

    // this isn't passing the session ID or the session name, so of course it's not going to get the correct token..
    // unless somehow this token is used w/ the login call so the server associated it that way..? but loging doesn't need a csrf token..
    //
    // SO, change this to pass the damn session data
    // BUT, i'm curious why the token passed back from login doesn't work.. try using that again..
    //
    // or pass cookie via xmlhttprequest headers using the sessionid & sessionname..? but browsers don't allow that
      ...
      try {
        if (token_request.readyState == 4) {
          ...
          if (token_request.status != 200) { // Not OK
            ...
            //console.log('JDRUPAL get csrf token setting sessid to', token);
            Drupal.sessid = token;
            if (options.success) { options.success(token); }
          }
        }
...
function system_connect(options) {
  ...
  try {
    ...
    var system_connect = {
      ...
      success: function(data) {
        try {
          //console.log('JDRUPAL system connect setting Drupal.user to', data.user);
    ...
    Drupal.services.call({
        ...
        success: function(data) {
          try {
            ...
            //console.log('JDRUPAL user logout resetting Drupal.user');
            Drupal.user = drupal_user_defaults();
            Drupal.sessid = null;
            system_connect({
                success: function(result) {
